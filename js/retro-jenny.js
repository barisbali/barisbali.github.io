document.addEventListener('DOMContentLoaded', () => {
    ...
    if (chatContainer) {
        // 1. Gömülü sohbet HTML'ini yükle
        fetch('assets/chatbot/chat-block.html')
            .then(response => response.text())
            .then(data => {
                chatContainer.innerHTML = data;

                // 2. Gerekli elemanları seç
                const messagesContainer = document.getElementById('chat-block-messages');
                const chatInput = document.getElementById('chat-block-input');
                const sendButton = document.getElementById('chat-block-send');

                // 3. Jenny'nin kişilik ve yanıt mantığını tanımla
...
const jenny = {
    ...
    generateResponse: function(input) {
        ...
        return response;
                ...
                // 4. Mesaj ekleme fonksiyonları
                const addMessage = (text, type) => {
                    const message = document.createElement('div');
                    message.className = `message ${type}-message`; // 'bot-message' veya 'user-message'
                    
                    if (type === 'bot') {
                         const avatar = document.createElement('img');
                         avatar.src = 'assets/images/botpicture.png';
                ...
                const handleSendMessage = () => {
                    ...
                    setTimeout(() => {
                        const response = jenny.generateResponse(text);
                ...
                // Başlangıç mesajı
                setTimeout(() => addMessage("Like, totally ready to chat! Ask me anything!", 'bot'), 500);
            });
    }
});